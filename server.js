...
const authenticateToken = (req, res, next) => {
    ...
    if (!token) {
        console.log('Auth Error: No access token provided');
...
app.post('/attendance/:fileId', authenticateToken, async (req, res) => {
    console.log(`Save Attendance Request: User=${req.user.username}, Role=${req.user.role}, FileId=${req.params.fileId}`);

    // Allow Student OR Admin
    if (req.user.role !== 'student' && req.user.role !== 'admin') {
        console.error(`Permission Denied: Role '${req.user.role}' is not allowed to save attendance.`);
        return res.status(403).json({ error: 'Permission denied: User must be student or admin' });
    }
    ...
    try {
        ...
        if (req.user.role === 'admin') {
            ...
            // Ideally we might want to respect branches not in the payload, but usually payload has all
            await supabase.from('attendance_records').delete().eq('file_id', fileId);
        } else {
        ...
        // For Admin, `student_branch` should come from the record data, not req.user.branch
        ...
        // If Admin saves, we might want to just mark it complete? Or check logic?
        // Let's keep the logic: checks unique branches submitted.
